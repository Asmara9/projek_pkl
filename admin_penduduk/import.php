...
<body>
    ...
    //trigger post dari button name=importpenduduk di halaman index.php
    if (isset($_POST['imporpenduduk'])) {
        ...
        for ($j = 2; $j <= count($data_excel); $j++) {
            ...
            $cekpenduduk = mysqli_query($koneksi, "SELECT nik FROM tbl_penduduk WHERE nik='$nik'") or die(mysqli_error($koneksi));

            $rvd = mysqli_num_rows($cekpenduduk);
            if ($rvd > 0) {
            } else {
                $kosong = "";

                $tambahpenduduk = mysqli_query($koneksi, "INSERT INTO tbl_penduduk VALUES ('', '$nik','$penduduk','$ketua','$kontak','$alamat')") or die(mysqli_error($koneksi));

                $hapuspendudukkosong = mysqli_query($koneksi, "DELETE FROM  tbl_penduduk WHERE nik='$kosong'") or die(mysqli_error($koneksi));

                $tambahpenggunapenduduk = mysqli_query($koneksi, "INSERT INTO tbl_pengguna VALUES ('$nik','$pass','$ketua','$st_pengguna')") or die(mysqli_error($koneksi));

                $hapus_pg_pendudukkosong = mysqli_query($koneksi, "DELETE FROM tbl_pengguna WHERE username='$kosong'") or die(mysqli_error($koneksi));
            }
        }
        unlink($target_file);
    ?>
        ...
        <script>
            swal("Berhasil", "Semua Data penduduk Berhasil di Import", "success");

            setTimeout(function() {
                window.location.href = "../admin_master_penduduk";
            }, 2000);
        </script>
    <?php
    }
